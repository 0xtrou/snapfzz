...
impl SystemComponent for AgentScopeComponent {
    ...
    fn is_installed(&self) -> bool {
        match self.run_uv(["run", "python", "-c", "import agentscope"]) {
...
mod tests {
    ...
    async fn t32_agentscope_resolve_has_expected_component_info() {
        ...
        assert_eq!(info.name, "AgentScope Runtime");
        assert_eq!(info.license, "Apache-2.0");
        assert_eq!(info.install_path, "/tmp/runtime/packages/agentscope");
