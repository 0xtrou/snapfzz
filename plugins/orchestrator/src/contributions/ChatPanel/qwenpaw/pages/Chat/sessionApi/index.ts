// Ported from qwenpaw/console/src/pages/Chat/sessionApi/index.ts — adapted for Snapfzz plugin context.
// SURGICAL EDIT (Phase 2f): import setActiveSessionId from adapter to keep adapter's
// _currentSessionId in sync with qwenpaw's window.currentSessionId writes.
// Path: sessionApi → Chat → pages → qwenpaw → ChatPanel → adapter (4 levels up)
import { setActiveSessionId } from '../../../../adapter';
import {
...
class SessionApi implements IAgentScopeRuntimeWebUISessionAPI {
  ...
  private createEmptySession(sessionId: string): ExtendedSession {
    ...
    // SURGICAL EDIT (Phase 2f): keep adapter in sync with window.currentSessionId
  ...
  private updateWindowVariables(session: ExtendedSession): void {
    ...
    // Persist the active Spark session id so a reload can restore the same chat.
    // Matches the earlier SessionIdBridge that was dropped in the port slimming.
    try {
      if (typeof localStorage !== "undefined" && session?.id) {
        localStorage.setItem("snapfzz.chat.session.v1", session.id);
      }
    } catch { /* privacy mode / quota — silent */ }
  }
  ...
  private applyChatsToSessionList(
    ...
    if (this.preferredChatId) {
      ...
      if (idx > 0) {
        ...
        // Stale — session no longer exists backend-side; clear persisted key.
        try { localStorage.removeItem("snapfzz.chat.session.v1"); } catch { /* ignore */ }
      }
    }
