// Ported from qwenpaw/console/src/api/modules/chat.ts — adapted for Snapfzz plugin context.
// SURGICAL EDIT (Phase 2a): uploadFile awaits async getApiUrl; filePreviewUrl returns
// a relative path (no token appended) since plugin auth is header-based not query-param.
...
export const chatApi = {
  ...
  uploadFile: async (file: File): Promise<ChatUploadResponse> => {
    ...
    // getApiUrl is async in plugin context
    const url = await getApiUrl("/console/upload");
  ...
  /**
   * Returns a relative /api/files/preview/... URL.
   * In plugin context we cannot resolve the async base URL synchronously,
   * so we return a path-only URL. The fetch will be relative to the current origin.
